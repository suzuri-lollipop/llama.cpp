...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    if (!graph_reuse_disable && res->can_reuse(gparams)) {
        ...
        // note: even if the previous graph is still being computed, it is safe to call set_inputs
        //   without synchronizing - graph inputs are allocated in the CPU backend and the scheduler
        //   copies them out before the async compute returns
